...
begin
  ...
  update public.rides r set passenger_id = v_current_id where r.passenger_id = v_target_id;
  update public.ride_requests rr set passenger_id = v_current_id where rr.passenger_id = v_target_id;

  select d.* into v_driver from public.drivers d where d.id = v_target_id for update;
  if found then
    v_application_id := v_driver.application_id;
    update public.drivers d set application_id = null where d.id = v_target_id;
    ...
    update public.rides r set driver_id = v_current_id where r.driver_id = v_target_id;
    update public.ride_requests rr set offered_driver_id = v_current_id where rr.offered_driver_id = v_target_id;
    update public.ride_requests rr
      set declined_driver_ids = array_replace(rr.declined_driver_ids, v_target_id, v_current_id)
      where v_target_id = any(rr.declined_driver_ids);
    delete from public.drivers d where d.id = v_target_id;
  end if;

  update public.driver_applications da set user_id = v_current_id where da.user_id = v_target_id;

  delete from public.profiles p where p.id = v_target_id;
